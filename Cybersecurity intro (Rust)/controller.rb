...
module Access
	...
	def authenticate(user, passwd)
		assign_session user if get_user2(user, passwd)
	end
...
module User
	...
	def register(user, filename, file, password, confirm)
		...
		return false if user !~ /(^[a-zA-Z0-9."()\s\w]+$)/
        return false if password !~ /(^[a-zA-Z0-9."()\s\w]+$)/
        return false if confirm !~ /(^[a-zA-Z0-9."()\s\w]+$)/
		return false if user !~ /(^[a-zA-Z0-9."()\s\w]+$)/
        return false if password !~ /(^[a-zA-Z0-9."()\s\w]+$)/
        return false if confirm !~ /(^[a-zA-Z0-9."()\s\w]+$)/
	...
	def get_user(user)
		return nil if user !~ /(^[a-zA-Z0-9."()\s\w]+$)/
		return nil if user !~ /(^[a-zA-Z0-9."()\s\w]+$)/
		query = %{
	...
	def update_prefs(user, session, description, token)
		return false if token != @toks[user] 
		return false if session != @sessions[user]
		return false if description !~ /(^[a-zA-Z0-9."()\s\w]+$)/
        return false if user !~ /(^[a-zA-Z0-9."()\s\w]+$)/
        return false if description !~ /(^[a-zA-Z0-9."()\s\w]+$)/
        return false if user !~ /(^[a-zA-Z0-9."()\s\w]+$)/
		query = %{
...
module Posts
	...
	def publish_post(user, session, content, token)
		return false if token != @toks[user] 
		return false if session != @sessions[user]
		return false if content !~ /(^[a-zA-Z0-9."()\s\w]+$)/
        return false if content !~ /(^[a-zA-Z0-9."()\s\w]+$)/
        return false if user !~ /(^[a-zA-Z0-9."()\s\w]+$)/
        return false if user !~ /(^[a-zA-Z0-9."()\s\w]+$)/
